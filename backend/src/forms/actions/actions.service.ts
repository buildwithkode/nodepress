...
import { ActionDef, FormField } from '../types/form-field.types';
...
export class ActionsService {
  ...
  async run(actions: ActionDef[], data: Record<string, unknown>, fields?: FormField[]): Promise<void> {
    await Promise.allSettled(
      actions.map((action) => {
        if (action.type === 'email')   return this.emailAction.handle(action, data, fields);
